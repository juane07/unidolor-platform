...
// TEST ROUTE
router.get('/test-detail-xyz', async (req, res) => {
  console.log('TEST DETAIL XYZ HIT');
  res.json({ success: true, message: 'Test detail route works from coreApi' });
});

console.log('coreApi.js loaded, test route registered');
...
//_______________________________ Institutional FAQ (admin panel) _______________________
...
router.route('/faq/list').get(catchErrors(institutionalFAQController.listFAQ));
