...
import { DatePicker, TimePicker, Input, Form, Select, InputNumber, Switch, Tag } from 'antd';
...
function FormElement({ field, feedback, setFeedback }) {
  ...
  if (customFormItem) return <>{customFormItem}</>;
    ...
    const getValueFromEvent = field.type === 'time'
      ? (value) => (value && value.format ? value.format('HH:mm') : value)
      : undefined;
    const getValueProps = field.type === 'time'
      ? (value) => ({ value: value ? dayjs(value, 'HH:mm') : null })
      : undefined;
    return (
      <Form.Item
        label={translate(field.label)}
