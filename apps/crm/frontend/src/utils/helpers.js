...
/*
 Format money to display admin, using persisted money settings.
*/
export function formatCurrency(amount = 0) {
  let money = {};
  try {
    const settings = JSON.parse(localStorage.getItem('persist:root') || '{}')?.settings;
    money = settings ? JSON.parse(settings)?.money_format_settings || {} : {};
  } catch (e) {
    money = {};
  }
  const formatted = currency(amount, {
    separator: money.thousand_sep || ',',
    decimal: money.decimal_sep || '.',
    symbol: '',
    precision: money.cent_precision ?? 2,
  }).format();
  return money.currency_position === 'before'
    ? (money.currency_symbol || '$') + ' ' + formatted
    : formatted + ' ' + (money.currency_symbol || '$');
}
